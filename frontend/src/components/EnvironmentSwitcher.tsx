...
import { IconAdd, IconCaretDown, IconCheck, IconClose, IconDelete, IconEdit } from './icons'
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
        <div className="bg-navy-900 border border-navy-700 rounded-xl shadow-2xl w-[560px] max-h-[85vh] flex flex-col">
          <div className="flex items-center justify-between px-5 py-4 border-b border-navy-800">
            ...
            <button onClick={() => { setShowManage(false); cancelEdit() }} className="text-white/60 hover:text-white">
              <IconClose size={15} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              ...
              <div key={env.id} className={`flex items-center justify-between p-3 rounded-lg border ${env.is_active ? 'border-dblue-500 bg-dblue-900/20' : 'border-navy-700 bg-navy-800'}`}>
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {env.is_active && <span className="text-xs text-primary font-semibold">ACTIVE</span>}
                  </div>
                  <p className="text-xs text-white/60">{env.host}:{env.port} · {env.auth_type}</p>
                </div>
                <div className="flex items-center gap-1">
                  {!env.is_active && (
                    <button
                      ...
                      className="px-2 py-1 text-xs text-primary hover:text-white hover:bg-dblue-600 rounded transition-colors disabled:opacity-50"
                    >
                      {activating === env.id ? <Loader2 size={12} className="animate-spin" /> : <IconCheck size={12} />}
                    </button>
                  )}
                  <button onClick={() => startEdit(env)} className="p-1 text-white/60 hover:text-white rounded transition-colors">
                    <IconEdit size={12} />
                  </button>
                  <button onClick={() => handleDelete(env)} className="p-1 text-white/60 hover:text-red-400 rounded transition-colors">
                    <IconDelete size={12} />
                  </button>
                </div>
              </div>
            ...
            {envs.length === 0 && !editing && (
              <p className="text-xs text-white/60 text-center py-4">No environments saved. Add one below.</p>
            )}

            {/* Add/Edit form */}
            <div className="border border-navy-700 rounded-lg p-4 space-y-3">
              <h3 className="text-xs font-semibold text-white/60">{editing ? `Edit: ${editing.name}` : 'Add Environment'}</h3>
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="block text-xs text-white/60 mb-1">Auth Type</label>
                ...
                {form.auth_type === 'password' ? (
                  ...
                  <>
                    <div className="col-span-2">
                      <label className="block text-xs text-white/60 mb-1">Personal Access Token</label>
                      <input type="password" value={form.pat} onChange={e => setForm(f => ({ ...f, pat: e.target.value }))}
                        className="w-full bg-navy-800 border border-navy-700 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-dblue-400" />
                    </div>
                    <div className="col-span-2">
                      <label className="block text-xs text-white/60 mb-1">Project ID (Cloud only)</label>
                ...
                <div className="col-span-2 flex items-center gap-2">
                  ...
                  <label htmlFor="ssl" className="text-xs text-white/70">Use SSL/HTTPS</label>
                </div>
              </div>
              <div className="flex gap-2">
                {editing && (
                  <button onClick={cancelEdit} className="px-3 py-1.5 text-xs text-white/60 hover:text-white rounded border border-navy-700 transition-colors">
                    Cancel
                  </button>
                )}
                <button
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex items-center gap-1.5 px-2 py-1 rounded text-white/70 hover:text-white hover:bg-sidebar-accent transition-colors text-xs border border-sidebar-border"
      >
        <Server size={13} />
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-52 bg-navy-900 border border-navy-700 rounded-lg shadow-xl z-50 overflow-hidden">
          <div className="px-3 py-2 border-b border-navy-800">
            <p className="text-xs font-semibold text-white/60">Environments</p>
          </div>
          {envs.length === 0 && (
            <p className="px-3 py-3 text-xs text-white/50">No environments saved</p>
          )}
          {envs.map(env => (
            <button
              key={env.id}
            ...
            >
              <span className={env.is_active ? "text-primary font-semibold" : "text-white/70"}>{env.name}</span>
              <span className="text-white/40">{env.host}</span>
            </button>
          ))}
          <div className="border-t border-navy-800">
            <button
              onClick={() => { setOpen(false); setShowManage(true) }}
              className="w-full flex items-center gap-2 px-3 py-2 text-xs text-white/70 hover:text-white hover:bg-sidebar-accent transition-colors"
            >
              <IconAdd size={12} /> Manage Environments
            </button>
          </div>
        </div>
      )}
