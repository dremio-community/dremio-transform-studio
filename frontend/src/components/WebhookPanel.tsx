...
import { IconCheck, IconCopy, IconRefresh, IconWarning } from './icons'
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="shrink-0 p-1.5 rounded text-white/60 hover:text-primary hover:bg-dblue-50 transition-colors"
...
def trigger_pipeline():
  ...
  return (
    <div className="p-4 space-y-5">
      {/* Header */}
      <div>
        <p className="text-xs font-semibold text-white/30 uppercase tracking-wider">Webhook Trigger</p>
      ...
      {/* Webhook URL */}
      <div>
        <p className="text-xs font-medium text-white/30 mb-1.5">Webhook URL</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <p className="text-xs font-medium text-white/30">cURL Example</p>
          <CopyButton text={curlExample} />
        </div>
        <pre className="text-xs font-mono bg-navy-950 text-white/70 rounded-lg p-3 overflow-x-auto whitespace-pre-wrap break-all leading-relaxed">
          {curlExample}
        </pre>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <p className="text-xs font-medium text-white/30">Airflow / Python Example</p>
      ...
      <div className="bg-surface-50 border border-surface-200 rounded-lg p-3">
        <p className="text-xs font-semibold text-white/30 mb-2">Request Body (optional)</p>
        <div className="space-y-1.5 text-xs text-white/30">
          <div>
            <code className="font-mono text-dblue-600">param_values</code>
            <span className="text-white/60"> — dict of parameter values to substitute</span>
          </div>
          <div>
            <code className="font-mono text-dblue-600">async_run</code>
            <span className="text-white/60"> — </span>
            <code className="font-mono">true</code>
            <span className="text-white/60"> (default) returns job_id immediately, </span>
            <code className="font-mono">false</code>
            <span className="text-white/60"> waits for completion</span>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-surface-200 pt-4">
        {!confirmRegen ? (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-xs text-white/40 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors w-full"
          >
            <IconRefresh size={12} /> Regenerate Token
          </button>
        ) : (
          <div className="bg-red-50 border border-red-200 rounded-lg p-3 space-y-3">
            <div className="flex items-start gap-2">
              <IconWarning size={13} className="text-red-500 shrink-0 mt-0.5" />
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 px-3 py-1.5 text-xs text-white/40 hover:text-surface-700 transition-colors"
              >
                Cancel
              </button>
              <button
